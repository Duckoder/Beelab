...
import { getAllCategory, getAllColor, getAllSupplier, addNewProduct } from '~/service/ApiService';
...
function CreateProduct() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const supliersData = await getAllSupplier();

        setCategories(categoriesData);
        setColors(colorsData);
        setSuppliers(supliersData);
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const updateData = { ...data, displayImage: URL.createObjectURL(selectedFile), discountPercent: 0, status: 1 };

      await addNewProduct(updateData);
